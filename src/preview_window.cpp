...
static LRESULT CALLBACK proc(HWND hwnd,UINT msg,WPARAM w,LPARAM l){switch(msg){
 ...
 case WM_KEYDOWN:if(l&(1L<<30))return 0;if(w==VK_ESCAPE){escapeHeld=true;actions.push_back({PreviewAction::Close});closePreviewWindow();}else if(w==VK_BACK)actions.push_back({PreviewAction::Back});else if(w=='W')actions.push_back({PreviewAction::Wet});else if(w=='O')actions.push_back({PreviewAction::Dock});return 0;
 case WM_LBUTTONDOWN:{auto r=fit(hwnd);float s=(r.right-r.left)/20.f;if(s<=0)return 0;float x=((short)LOWORD(l)-r.left)/s,y=((short)HIWORD(l)-r.top)/s;for(size_t i=0;i<snapshot.objects.size();i++){auto& o=snapshot.objects[i];if(o.kind=="chest"&&!o.target.empty()&&x>=o.x-.65f&&x<=o.x+.65f&&y>=o.y-.9f&&y<=o.y+.65f){actions.push_back({PreviewAction::Select,(int)i});break;}}return 0;}
...
void updatePreviewWindow(const RoomArt& next,const Snapshot& s,const std::string& room,int depth,const std::string& condition,const std::string& status){if(!popup)return;diagnostic=depth==0;auto newTitle="Recursed++ | Depth "+std::to_string(depth)+" | "+room+" | "+condition;bool changed=false;if(newTitle!=title){title=newTitle;SetWindowTextW(popup,wide(title).c_str());changed=true;}auto nextInfo=status+"\n"+next.note;if(info!=nextInfo){info=nextInfo;changed=true;}if(art.revision!=next.revision||art.pixels.empty()){art=next;snapshot=s;changed=true;}if(changed)InvalidateRect(popup,nullptr,FALSE);}
}
