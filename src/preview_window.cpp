...
void updatePreviewWindow(const RoomArt& next,const Snapshot& s,const std::string& room,int depth,const std::string& status){if(!popup)return;diagnostic=room=="ACTIVE ROOM - NATIVE TEST";auto newTitle="Recursed++ | Depth "+std::to_string(depth)+" | "+room;bool changed=false;if(newTitle!=title){title=newTitle;SetWindowTextW(popup,wide(title).c_str());changed=true;}auto nextInfo=diagnostic?status+"\n"+next.note:status;if(info!=nextInfo){info=nextInfo;changed=true;}if(art.revision!=next.revision||art.pixels.empty()){art=next;snapshot=s;changed=true;}if(changed)InvalidateRect(popup,nullptr,FALSE);}
}
