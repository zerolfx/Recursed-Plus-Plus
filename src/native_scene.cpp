...
static void* entity(const Object& o){
 uintptr_t ctor=0;size_t bytes=0;
 if(o.kind=="chest"){ctor=0x410fe0;bytes=0x7c;}else if(o.kind=="box"){ctor=0x410210;bytes=0x50;}else if(o.kind=="key"){ctor=0x416210;bytes=0x50;}else if(o.kind=="lock"){ctor=0x416790;bytes=0x50;}else if(o.kind=="crystal"||o.kind=="diamond"||o.kind=="ruby"){ctor=0x412580;bytes=0x5c;}else return nullptr;
 void* p=allocate(bytes);if(!p)return nullptr;
 if(o.kind=="chest"){
 ...
 using Add=bool(__thiscall*)(void*,void*,bool);fn<Add>(0x41c130)((void*)room,p,false);
...
void clearNativeScene(){
 ...
 entities.clear();rotating.clear();stack.clear();signature.clear();source=0;elapsed=0;
}
bool prepareNativeScene(uintptr_t liveHost,const Snapshot& s,const std::string& key,int depth){
 ...
 for(const auto& o:s.objects)if(o.kind!="player"&&o.kind!="chest"&&o.kind!="box"&&o.kind!="key"&&o.kind!="lock"&&o.kind!="crystal"&&o.kind!="diamond"&&o.kind!="ruby"){error="Native scene does not yet support "+o.kind;return false;}
 std::array<int,300> indices{};for(size_t i=0;i<300;i++){indices[i]=tileIndex(liveHost,s.tiles[i]);if(indices[i]<0||indices[i]>4096){error="Native tile definition missing: "+s.tiles[i].definition;return false;}}
 std::ostringstream stamp;stamp<<liveHost<<key<<depth;for(size_t i=0;i<300;i++)stamp<<','<<s.tiles[i].kind<<':'<<indices[i];for(const auto& o:s.objects)stamp<<'|'<<o.kind<<o.target<<o.x<<','<<o.y<<o.global;
 ...
 stack.assign((unsigned char*)begin,(unsigned char*)end);for(int i=0;i<depth;i++)stack.insert(stack.end(),28,0);
 ...
 for(const auto& o:s.objects)if(o.kind!="player"&&!entity(o)){error="Entity allocation failed";clearNativeScene();return false;}
