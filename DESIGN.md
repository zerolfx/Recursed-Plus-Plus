...
The current implementation supports eight levels, Backspace to go back, Esc to close, right click to close a pinned preview, and O to switch presentation without losing the path. A close leaves the chest under the pointer dismissed until the pointer reaches another one. The separate window displays depth and destination. A short underline identifies the hovered chest, and inside a pinned preview only what a click there would open; persistent debugging rectangles are not part of the presentation. Inside a pinned inline preview, hovering a chest or flame adds the depth and room a click there opens to the depth line, computed by the same step the click takes, so the line cannot promise a different destination than the click reaches.
...
- Return portals resolve existing ancestors in the real room stack; their tiles and object positions are resampled. A room puts its globals aside when it is left and restores them when it is returned to, so an ancestor reached through a flame shows those globals as well.
- A return portal leads where the game's exit rule sends it, not always to the room below. That room takes the player back only if what its player went in through is carried out, or is the room's again when the player re-attaches: ahead of the player in the room, or among the globals the room restores first. A restore puts each global back where it was saved unless a solid tile or a locked lock is in the way, and a refused one stays put aside. Otherwise the game leaves the whole stack behind for a new timeline in `reject`, when that was a chest, or `threadless`, built fresh and dry as the only room of its stack: the globals saved under its name, the timeline's colours, and no flame. A script without that room gets an empty one. Walking out moves globals and the held item along the way, and a green flame takes nothing through, so a portal further out is predicted by walking out of every room in between, remembering which flames on the way were green. The outermost room has no flame: the game makes one only in a room built with another below it.
- Carried-item branches and successive hypothetical entries require additional entry-context modeling.
- Jars preserve instances, and cauldrons switch timeline. A room name alone cannot represent their state.
- A cauldron leads into the timeline it names. Switching puts the whole stack being played aside under the timeline being played, after the room on top has put its globals aside, and takes back the stack the named timeline was left with. Its top room takes back the globals saved under its own name, which a room of that name in any timeline may have taken or added to, and its player then needs the way it went out, normally the cauldron it left through: missing, the switch ends in a paradox, `threadless` for a cauldron. A timeline never left has no stack, so its first room, named after it, is built fresh, dry and alone, with the globals saved under its name. Into the timeline being played, the same stack comes back, and only a global cauldron the room refuses to take back can make that a paradox. What the player carries goes with them.
- A preview reads a saved stack where the game keeps it, without holding on to any of its pointers, and counts that timeline's depth from its first room. A flame in a room returned to walks out through that stack the way the game would once the switch has happened. The path is the history of one hypothetical walk. A cauldron inside a preview goes back to the room the path was last in within that timeline, since leaving puts that room on top, and shows that step again as a new one rather than cutting the path back to it, so the rooms other timelines were left in stay on the path for later cauldrons. Into the timeline being played otherwise, it goes to the room being played, whose player left through the path's first cauldron, with the globals that room takes back. After a flame walk that ended in a paradox, the timeline walked out of is left with the room it stopped in on top, still without its way back, so a cauldron back into it is not offered. The Lua room function also receives what the player holds, which a preview passes as nothing.

Fresh destinations reuse native spawn collision placement. They do not run the full ongoing physics update. Outer snapshots preserve existing positions, except that the globals an outer room takes back are placed the way its restore places them. Water follows game state without a manual override. The game remains active while previews are open; a focused separate preview window suppresses game keyboard queries.
...
The preview owns its Room, entities, Renderer, and copied stack. It borrows level metadata only while that level is active. The renderer takes its colours from the timeline named in the host, so a paradox room's private host copy names the paradox timeline instead of the one being played, and a room in the timeline a cauldron leads to names that one. Apart from the top room, the renderer reads only how many rooms the stack holds, for the background, so the copy holds as many as the room's own timeline would. Native allocations use the game's allocator and matching destructors. Room numbering and preview random-number consumption are isolated. Visual transforms advance on a private clock while room physics is frozen.
...
Each additional native draw audits the real stack, Room data, and entity common fields. A failed audit disables native rendering for the process. GPU caches and all entity-specific fields are not covered by that audit. Scene changes invalidate previews, and native resources are rebuilt when the target snapshot changes. Both inline and separate-window presentations use the same refreshed snapshot. Nested live/global entries retain source identity and are revalidated; deletion returns to the parent and changed liquid conditions refresh the branch. Window click actions retain the displayed object instead of a potentially stale vector index, together with the view it was clicked in; a click for a view no longer shown is dropped. Outer steps take their depth from the actual stack, and each inner step is one deeper than the step it was opened from, including paths that go outside and then inside again. Steps store depth relative to the room being played, not the engine's absolute depth, because walking out through a flame builds no room: a pinned chest carried out keeps its preview, and its depths stay right. A paradox leaves the rooms being counted from behind, so a paradox room reads Paradox and the rooms inside it count from it. A step through a flame of the stack keeps which room's flame it was and is predicted again every frame, so it turns into the paradox room and back as the chest it depends on moves. A flame back to the previous step is Back, and any new step clears Forward.
...
- Hypothetical paths that change a saved stack before switching back to it, such as walking out into a paradox and taking a cauldron home, and events a saved room still has queued when it is returned to.
