...
int main(){
 ...
 // A record must reach the "ring" entry of its asset; drawMesh is the only place
 // that split happens, and falling back would draw the schematic dot instead.
 auto ring=peek::loadSnapshot("runtime","missions/peek-lab","keyroom",false);assert(ring.error.empty());
 ring.objects.erase(std::remove_if(ring.objects.begin(),ring.objects.end(),[](const peek::Object& o){return o.kind!="record";}),ring.objects.end());
 assert(ring.objects.size()==1);
 assert(peek::renderRoomArt("runtime",ring,2.01).note.find("schematic")==std::string::npos);
